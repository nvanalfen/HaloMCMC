...
def log_prob(theta, inv_cov, x, y, model_instance, halocat, rbins, split):
    ...
    cut = model_instance.mock.galaxy_table
    ...
    posy = cut["y"]
    posz = cut["z"]
    axis_x = cut["galaxy_axisA_x"]
    axis_y = cut["galaxy_axisA_y"]
    axis_z = cut["galaxy_axisA_z"]
    #sat_coords = np.array( [satx,saty,satz] ).T
    coords = np.array( [posx,posy,posz] ).T
...
if __name__ == "__main__":
    ...
    omega_df = pd.read_csv("static_halos_all_galaxy_omega_radial_AS_seed=132358712.csv", index_col=False)
    subhalo_df = pd.read_csv("static_halos_all_halo_omega_radial_AS_seed=132358712.csv", index_col=False)
    ...
    #cov = np.load("cen_pos_sat_shape_cov.npy")
    omega_df = np.array(omega_df).T
    #omega_df = omega_df + ( np.random.normal(0,2,omega_df.shape) * omega_df )
    cov = np.cov( omega_df[:split, :] )
    inv_cov = np.linalg.inv(cov)
    # Include the factor from the paper
    inv_cov *= (200-19-2)/(200-1)

    try:
        start = time.time()
        f_name = os.path.join("const_full_front_results","MCMC_"+job+".h5")
        ...
        sampler.run_mcmc(p0, 3000, store=True, progress=True)
